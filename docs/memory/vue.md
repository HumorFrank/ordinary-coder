...
| 类型             | 典型场景         | 性能考量         | 是否常用 |
| ---------------- | ---------------- | ---------------- | -------- |
| 单文件组件 (SFC) | 几乎所有业务组件 | 标准性能         | ⭐⭐⭐⭐⭐    |
| 纯对象组件       | 简单封装、测试   | 略快于SFC        | ⭐⭐⭐      |
| 函数式组件       | 纯展示组件       | 无状态实例开销小 | ⭐⭐       |

2️⃣ 加载方式

| 类型     | 典型场景           | 性能考量             | 是否常用 |
| -------- | ------------------ | -------------------- | -------- |
| 同步组件 | 首屏核心组件       | 立即加载，增大bundle | ⭐⭐⭐⭐⭐    |
| 异步组件 | 路由懒加载、大弹窗 | 减小bundle，首次慢   | ⭐⭐⭐⭐⭐    |

```ts [异步组件]
// 异步组件是为另一个组件提供的包装器，来让被包装的组件可以进行懒加载。
...
| 类型         | 典型场景               | 性能考量         | 是否常用 |
| ------------ | ---------------------- | ---------------- | -------- |
| 动态组件     | Tab切换、动态表单      | 有重建/销毁开销  | ⭐⭐⭐⭐⭐    |
| 递归组件     | 树形菜单、无限评论     | 注意最大深度限制 | ⭐⭐⭐      |
| 内联模板组件 | 动态生成模板（不推荐） | 运行时编译开销大 | ⭐        |
...
| ------------- | ------------------- | ------------ | -------- |
| 普通组件      | 日常业务逻辑        | 标准性能     | ⭐⭐⭐⭐⭐    |
| Suspense组件  | 协调多个异步依赖    | 改善加载体验 | ⭐⭐⭐      |
| KeepAlive组件 | 表单切换、路由缓存  | 内存换性能   | ⭐⭐⭐⭐     |
| Teleport组件  | 弹窗、通知、Tooltip | 最小性能影响 | ⭐⭐⭐⭐     |

5️⃣ 作用域

| 类型     | 典型场景           | 性能考量     | 是否常用 |
| -------- | ------------------ | ------------ | -------- |
| 全局组件 | UI库、高频通用组件 | 始终在内存中 | ⭐⭐⭐      |
| 局部组件 | 页面级私有组件     | 按需加载     | ⭐⭐⭐⭐⭐    |
...
| 类型                 | 典型场景        | 性能考量       | 是否常用 |
| -------------------- | --------------- | -------------- | -------- |
| defineAsyncComponent | 基础异步加载    | 标准异步性能   | ⭐⭐⭐⭐⭐    |
| Suspense + 异步组件  | 需要fallback UI | 更好的加载体验 | ⭐⭐⭐      |

7️⃣ 传送目标

| 类型               | 典型场景   | 性能考量         | 是否常用 |
| ------------------ | ---------- | ---------------- | -------- |
| Teleport to body   | 根层级弹窗 | 避免CSS层级问题  | ⭐⭐⭐⭐     |
| Teleport to 选择器 | 特定挂载点 | 灵活但需确保存在 | ⭐⭐⭐      |
...
✴️ 总结
- `异步组件 vs 动态组件`：异步控制什么时候加载组件，动态控制渲染哪个组件
- `Suspense vs 异步组件`：Suspense 管理多个异步组件的协调状态
- `KeepAlive vs Teleport`：KeepAlive 控制缓存，Teleport 控制渲染位置

#### 按使用场景分类

| 场景         | 推荐组件类型          | 示例                                          |
| ------------ | --------------------- | --------------------------------------------- |
| 路由页面     | 异步组件 + 路由懒加载 | `component: () => import('./Home.vue')`       |
| Tab切换      | 动态组件 + KeepAlive  | `<component :is="tab" /> + <KeepAlive> `      |
| 弹窗/通知    | Teleport + 异步组件   | `<Teleport to="body"> + defineAsyncComponent` |
| 树形菜单     | 递归组件              | `TreeItem`组件自调用                          |
| 权限按钮     | 高阶组件              | `withPermission(Button, 'delete')`            |
| 复杂表单缓存 | KeepAlive + 动态组件  | 切换标签页保留表单数据                        |
| 统计报表     | Suspense + 异步组件   | 等待多个图表API完成                           |
| 全局Toast    | Teleport + 全局组件   | `app.component('GlobalToast', Toast)`         |
| 动态表单     | 动态组件 + 渲染函数   | 根据`schema`动态生成                          |
| 无限加载     | 异步组件 + 递归       | 评论区懒加载子评论                            |
...
- `无状态逻辑`：普通函数即可。
  - 输入 → 输出，不保留任何中间信息。
  - 每次调用都是全新。
  - 是函数内部的临时变量，在函数执行完后就被垃圾回收了。
  - 只管逻辑处理，不记任何东西
- `有状态逻辑`：需要用组合式函数封装（在组件的生命周期内，该逻辑拥有并维护着至少一个跨多次函数调用而持久存在的内存数据。）
  - 数据不会随函数调用结束而销毁
  - 对象一直存在内存中，直到组件销毁
...
// 📍 情景 A：记得之前的值
let memory = 0;
function addOneWithMemory() {
  memory = memory + 1;
  return memory;
}

// 📍 情景 B：有状态（跨调用持久化）
const permanent = { count: 0 }; // 永久的存储空间

function update() {
  permanent.count++; // 修改的是永久存储
  return permanent.count;
}
// permanent 对象一直存在内存中，直到组件销毁

// 📍 情景 C：保留信息，同样的输入可能产生不同的输出
function createCounter() {
  let count = 0;  // ← 这就是“持久化的内存数据”
  return () => ++count;
}
...
// 📍 情景 D：持久化内存”
function useUser() {
  const user = ref(null);  // ← 这就是“持久化内存”
  const setUser = (u) => user.value = u;
  return { user, setUser };
}
```

> 组合式函数内部的 ref/reactive/普通变量，只要不是函数内部的临时变量，而是返回出去或被闭包引用的，都会成为“跨调用持久化的内存数据”。

2️⃣ 核心区别
| 类型         | 特征                                                                               | 例子                                                               |
| ------------ | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| 无状态的逻辑 | 输入参数 → 输出结果。每次调用独立，不记忆之前的结果，不影响外部世界。              | 格式化日期、数值计算、数组去重、发起一次 HTTP 请求（不缓存结果）。 |
| 有状态的逻辑 | 创建并维护一份随时间变化的数据。多次调用之间可以“记住”信息，并能触发组件重新渲染。 | 管理鼠标位置、维护倒计时、缓存 API 结果、管理表单校验状态。        |
...
| ------------------ | ---------- | -------------------------------- |
| ref                | ✅ 非常常见 | 解构后仍保持响应性               |
| computed           | ✅ 非常常见 | 本质也是 ref（只读）             |
| 普通函数           | ✅ 非常常见 | 用于修改内部状态                 |
| reactive           | ⚠️ 少见     | 解构会丢失响应性，一般不直接返回 |
| watch              | ❌ 不返回   | 副作用，在组合式函数内部执行即可 |
| readonly(ref)      | ✅ 常见     | 保护状态不被外部意外修改         |
| toRef/reactive属性 | ✅ 常见     | 将 reactive 属性转为 ref 返回    |

>  reactive在组合式函数中直接返回 reactive 对象（❌ 不推荐）
> - 1.解构会丢失响应性。
> - 2.不能重新赋值整个状态。
