...
::: tip 容器化技术
**容器化技术**将应用及其所有依赖打包成一个标准化的单元，确保在任何环境中都能一致运行。它是现代软件交付的基石。
:::
...
Docker 围绕三个核心概念：镜像（Image）、容器（Container）、仓库（Registry）。
...
## 参考资料
- [Docker 容器化](https://datawhalechina.github.io/easy-vibe/zh-cn/appendix/7-infrastructure-and-operations/docker-containers.html)
